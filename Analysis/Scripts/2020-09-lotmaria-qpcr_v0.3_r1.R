...
## *to do: R1*
## verify residuals plots 
## Move Fig 1 sample size annotations down to X axis
## Move Fig 1 significance asterisks down 
 ##to just above sample sizes
##note object for export of raw data!
 ## Samples.model.graph ##
...
## v0.1 Update 2022-03-12 Add second set of samples ##
 ## Kyle's qPCR for trypanosomatids, 8 more plates

## v0.2 update models to get emmeans with ratios
 ## change phrasing of response var from 'log1p(y)' to (log(y + 1))
 ## emmeans will autodetect this transformation

## v0.3 Implement Figure changes for R1 (J Invert Pathology)
####################
...
emm.test = emmeans(model.test, ~Treatment)
(contrast.test = contrast(emm.test, "trt.vs.ctrl"))
...
### Start with negative binomial model ###
m.quant =
...
## Poisson and negative binomial fits were miserable ##
## Try log-normal model instead
m.lnorm =  
...
# Chisq Df Pr(>Chisq)    
# Treatment       118.0790  8     <2e-16 ***
#   Cq.median_Actin   0.4105  1     0.5217 
...
## Adding raw data and time 0 mean##
t0.summ ## Initial count only 41K
Mean.t0 = 
  t0.summ %>%
  filter(Label == "TIME 0 PAR") 
...
Mean.samples.overall = 
  ...
  summarize(Median = median(Copies.median),
            ...
            Mean = mean(Copies.median),
            SD = sd(Copies.median),
            N = length(Copies.median),
            N.pos = length(Copies.median[Copies.median > 0]),
            Prevalence = N.pos/N)
Mean.samples.overall
...
# Treatment     Mean        SD
# <chr>        <dbl>     <dbl>
#   1 0-P       1275808.  1938818.
# 2 C-H        126653.   506188.
# 3 C-L       1040366.  2409993.
# 4 E-H          3193.    15411.
# 5 E-L        401534.  1318900.
# 6 N-H       1034284.  3226687.
# 7 N-L        727670.  1363663.
# 8 T-H        399649.  1465152.
# 9 T-L       6720876. 21485856.
...
p.ci.raw =
  p.ci.text + 
  ## annotate: Mean time 0 
  geom_hline(data = Mean.t0, 
                         aes(yintercept = Mean,
                             linetype = Label))+
...
ggsave("./Figures/lotmaria.emmeans.v1.2_addraw.pdf", 
       height = 3.5, width = 5) 
##This was fig 1 in initial submission

## R1: Move labels and asterisks down
p.ci ## plot without annotations

Sep.level = 1 ## spread it out 
pd = position_dodge(width = Sep.level)
pjd = position_jitterdodge(
  dodge.width = Sep.level,
  jitter.width = 0.2)
...
ggsave("./Figures/lotmaria.emmeans.v1.3_move.labs.pdf", 
       height = 3.5, width = 5) 
...
########## TABLE PREP #############
##################################
## Recode the model to get type = "response" contrasts as ratios##

m.recode = 
  glmmTMB(log(1 + Copies.median) ~ Treatment,
          data = Samples.full.model)

emm.recode = 
  emmeans(m.recode, ~Treatment, type = "response")

df.ratio = 
  contrast(emm.recode, "trt.vs.ctrl") %>% ## there's my ratio
  as_tibble() %>%
  rename(`Relative intensity` = ratio, 
         `T` = t.ratio, `P` = p.value) %>%
  tidyr::separate(contrast, 
                  into = c("Treat", "Baseline"), 
                  sep = " / ") %>%
...
(contrasts.table = 
  ...
  mutate(Compound =   plyr::mapvalues(Comp, 
                                      from = c("X", "C", "E", "N", "T"),
                                      to = c("None", "Carvacrol", "Eugenol", 
                                             "Cinnamaldehyde", "Thymol")),
         Concentration = plyr::mapvalues(Conc,
                                         from = c("0", "L", "H"),
                                         to = c("0", "Low", "High")),
         .after = Treatment) %>%
  filter(!is.na(`P`)) %>%
  dplyr::select(Compound, Concentration, `Relative intensity`:`P`)
)

# dplyr::select(Parasite.o, Compound.o, Conc.o,
#               ratio, SE.ratio, z.ratio, p.value) %>%
  
## Round values ##
N.col = ncol(contrasts.table)

contrasts.table
names(contrasts.table)

(contrasts.rounded = 
  contrasts.table %>% 
    mutate(across(c(`Relative intensity`, SE), signif, 3)) %>%
  mutate(across(c(df, `T`), round, 2)) %>%
  mutate(P = 
           ifelse(P < 0.001, "<0.001", signif(P,3))
  )
)

## Export table ##
write.xlsx(contrasts.rounded, file = "./Figures/Infection_table_v0.xlsx")
...
## Meta: mapvalues
 ## Parasite / Compound / Conc

unique(Samples.full.model$Compound)
Compound.abbs = c("C", "N", "E", "X", "T")
Compound.names = c("Carvacrol", "Cinnamaldehyde", "Eugenol",
                   "None",
                   "Thymol")

unique(Samples.full.model$Conc)
Conc.abbs = c("L", "H", "0")
Conc.names = c("Low", "High", "0")

unique(Samples.full.model$Parasite) ## OK

## Responses:
##Copies.median_actin; 
## Copies.median_lotmaria = Copies.median
Infection.export = 
  Samples.full.model %>%
  mutate(Concentration = 
           plyr::mapvalues(Conc,
                           ...
                           to = Conc.names),
         Compound = 
           plyr::mapvalues(Compound,
                           from = Compound.abbs, 
                           to = Compound.names) 
         ) %>%
  dplyr::select(Bee_ID, Compound.name = Compound, 
                Concentration, Cage,
                Copies.median_Lotmaria = Copies.median,
                Copies.median_Actin)

## Write data ##
write_excel_csv(Infection.export,
                "./Data/Processed/Infection_data.csv")
openxlsx::write.xlsx(Infection.export,
                     "./Data/Processed/Infection_data.xlsx")

######################################################################
### Extras: FAILED ATTEMPTS TO USE regrid() with lsmeans/emmeans ##
#######################################################################

## Regrid the model to let emmeans know there is a log1p link
model.regrid = 
  update(ref_grid(m.lnorm), tran = make.tran("genlog", 1))

# now use the regridded model in call to emmeans
emmeans(model.regrid, ~Treatment, type = "response")
contrast(regrid(emmeans(m.lnorm.re, ~Treatment),
                transform = "log"), 
         "trt.vs.ctrl")
...
# now use the regridded model in call to emmeans
emm.rg = emmeans(model.regrid, ~Treatment)
emm.rg
contrast(emm.rg, ~Treatment, "trt.vs.ctrl") ## log scale

emm.rg.tf = emmeans(model.regrid, ~Treatment, type = "response",
                    tran = make.tran("genlog", 1))
emm.rg.tf
contrast(emm.rg, type = "response")
## didn't work

pairwise.regrid = 
  update(contrast(emm.rg, type = "response", 
                "trt.vs.ctrl", ref = 2),
       tran = make.tran("genlog", 1))
## no luck, still on log scale

contrast(regrid(emmeans(m.lnorm, ~Treatment,
                type = "response"),
                tran = make.tran("genlog", 1))
)
...
emm.rg.tf = emmeans(model.regrid, ~Treatment, type = "response",
                    tran = make.tran("genlog", 1))
emm.rg.tf
pairs(emm.rg.tf, type = "response") ## nope

## one more try
m.lnorm
m.emm.rg <- regrid(emmeans(m.lnorm, "Treatment"), 
                   transform = "log")
confint(m.emm.rg, type = "response")
pairs(m.emm.rg)
pairs(m.emm.rg, type = "response")
...
m.emm.rg.lp1 <- 
  regrid(
    emmeans(m.lnorm, "Treatment", type = "response"),
         transform = make.tran("genlog", 1))

m.emm.rg.lp1

pairs(m.emm.rg.lp1) ## log scale

pairs(m.emm.rg.lp1, type = "response") ## still log scale

update(pairs(m.emm.rg.lp1, type = "response"),
       tran = make.tran("genlog", 1))
...
update(pairs(m.emm.rg.lp1, type = "response"),
       tran = my.tran) ## nothing doing

## no luck getting ratios
## try to recode model to auto-detect transformation

## Does this work with a simpler model?

data(pigs)
str(pigs)
m.pig = lm(conc ~ source + as.factor(percent), data = pigs)  
emm.pig = 
  emmeans(m.pig, "source")
contrast(emm.pig, "trt.vs.ctrl", type = "response")
# contrast    estimate   SE df t.ratio p.value
# soy - fish      9.47 2.33 23 4.059   0.0010 
# skim - fish    15.58 2.39 23 6.526   <.0001 

## regrid for ratios
piglog.emm.s <- regrid(emmeans(m.pig, "source"), 
                       transform = "log")
contrast(piglog.emm.s, "trt.vs.ctrl", type = "response")
##  contrast    ratio     SE df t.ratio p.value
# soy / fish   1.32 0.0911 23 3.984   0.0011 
# skim / fish  1.52 0.1014 23 6.298   <.0001 
## nb: regrid() worked but t-stats slightly altered
