...
#v0.2--
#Change pairwise contrasts to use 0-P group as reference
 ## this better matches qpcr analysis

#v0.3--
 ##Remove letters from fig legend color key
  ## base file:  ./Analysis/Figures/Hazards_v2.1_0p_contrast.pdf
  ## can use P + geom_text(show.legend = FALSE) 
  ## but ended up just fixing in Adobe to save some trouble
 ## new file: ./Analysis/Figures/Hazards_v2.2_r1_fix_legend.pdf
...
summary(fit)
broom::tidy(fit)
zph <- cox.zph(fit, transform = "identity") ## test proportional hazards
zph
car::Anova(fit)
# Df  Chisq Pr(>Chisq)    
...
CLD(emm.contrast) ## deprecated

(emm.contrast <-emmeans(fit, ~Treatment.o, type = "response")) 
## This gives relative hazard

emm.contrast.0p = contrast(emm.contrast, "trt.vs.ctrl", ref = 2)
emm.contrast.0p

## compare to non-response
(emm.logscale = emmeans(fit, ~Treatment.o))
contrast(emm.logscale, "trt.vs.ctrl", ref = 2) ## good, same

########### #############################
## First plot: all pairwise comparisons ###
###############################
...
CLD.df <- CLD(emm.contrast) ## deprecated
...
######
## Alternate plot: slightly less conservative, 
 ##compare all groups to group 0-P 
emm.contrast
emm.contrast.df = as.data.frame(emm.contrast)
emm.contrast.0p = contrast(emm.contrast, "trt.vs.ctrl", ref = 2)
...
emm.contrast.0p.df = 
  ...
  mutate(Treatment.o = str_replace_all(Treat, "\\(|\\)", ""), 
         .before = Treat) %>%
  #mutate(Treatment.o = )
  right_join(emm.contrast.df, by = "Treatment.o")

## Merge this to original data
## Format plot to match order of consumption plot
Df.n <- 
  Df.impute %>% ##CONTINUE TO MERGE TO ORIGINAL DATA
  ## THEN MERGE TO contrasts for graphic annotation
  group_by(Parasite.o, Treatment.o, Compound.o, Conc.o) %>%
  dplyr::count() ## next merge to CLD and emmeans

## Hitch contrasts & emmeans to other variables
### Merge to emmeans ##
contrasts.plus <-
  left_join(emm.contrast.0p.df, Df.n, by = "Treatment.o")
...
p.contrast = 
  ...
  theme(axis.text.x = element_text(angle = 30, hjust = 1,
                                   vjust = 1, color = "black")) +
  geom_text(aes(label = Stars, y = asymp.UCL),
            position = position_dodge2(width = 0.75),
            size = 8,
            vjust = 0.5) + 
  ## Bonus: give sample sizes along x-axis ##
...
ggsave("./Figures/Hazards_v2.1_0p_contrast.pdf", 
       height = 4, width = 5)

############ RESULTS TABLE ##################
## Second bonus: export table with z-values for contrasts
glimpse(contrasts.plus)
levels(contrasts.plus$Treatment.o)
...
contrasts.table = 
  ...
  rename(`Hazard ratio` = ratio,
         SE = SE.ratio,
         Z = z.ratio,
         P = p.value) %>%
  dplyr::select(-c(Parasite.o, Compound.o, Conc.o))
...
write.xlsx(contrasts.rounded, file = "./Figures/Survival_table_v0.xlsx")
...
P +
  ...
  xlab("Time (d)")
ggsave("Survival-curves-v0-facet.pdf",
       height = 7, width = 7)
...
Df.exp %>% ##Change dead-before to dead (new imputed column)
  ...
  write_excel_csv("Survival_data.csv")
...
Df.exp %>%
  dplyr::select(Parasite.treatment,
                ...
                Number,
                End_time:Dead) %>%
  write_excel_csv("Survival_data_named.csv")
