...
Df.impute <- 
  Df.c.ord %>%
  mutate(End_time = ifelse(is.na(End.time), 11, End.time),
         Dead = ifelse(is.na(End.time), 0, 1))
...
car::Anova(fit)
# Df  Chisq Pr(>Chisq)    
# Treatment.o  9 78.416  3.333e-13 ***
...
Df.exp %>%
  dplyr::select(Parasite:Dead_before, End.time) %>%
