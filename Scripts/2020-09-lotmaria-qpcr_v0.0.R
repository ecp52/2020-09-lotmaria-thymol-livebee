...
ggsave("./Figures/exploratory_boxplot_lotmaria_v0.pdf",
       height = 3, width = 5)
...
ggsave("./Figures/Raw.vs.corrected.SD.v0.pdf", height = 3, width = 5)
## Looks like correction is increasing variance, not decreasing it
## And only explains 2% of the variance!!
...
ggsave("./Figures/exploratory_boxplot_lotmaria_v0.1.pdf",
       height = 4, width = 6)
...
ggsave("./Figures/lotmaria.boxplot.v1.1.no.inoculated.pdf",
       height = 4, width = 7)
...
car::Anova(m.lnorm.actin.re) ## Actin p = 0.35
...
## How to get this is proper order?
Treatments.meta =
...
p.ci = 
  ...
  theme(axis.text.x = element_text(color = "black",
                                  ...
                                  # vjust = 0.5, color = "black"))
p.ci
ggsave("./Figures/lotmaria.emmeans.v0a.vjust1.pdf", height = 3.5, width = 5)
...
## Let's do pairwise comparisons between each treatment and control ##
?`contrast-methods`
...
## Now how to merge this back to emmeans data?
pairwise.starry.treat = 
  pairwise.starry %>%
  tidyr::separate(contrast, 
                  into = c("Treat", "Baseline"), 
                  sep = " - ") %>%
  mutate(Treatment = str_sub(Treat, 2, ???2),
         .before = Treat) %>%
...
ggsave("./Figures/lotmaria.emmeans.v0.1a.stars.vjust1.pdf", 
       height = 3.5, width = 5)
